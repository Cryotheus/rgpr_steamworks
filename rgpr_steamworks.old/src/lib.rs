...
impl Steamworks {
	...
	pub fn init(config: SteamworksInitConfig) -> Result<Self, Error> {
		...
		unsafe {
			...
			if let Some(error) = Error::init(result_cenum, err_msg) {
				return Err(error);
			}
		...
		let arc = Arc::new(SteamworksInner {
			...
			call_manager: mutex(call_manager),
	...
	/// The App ID provided during [`init`](Self::init).
	...
	/// Returns `None` if the interface was not enabled during [`init`](Self::init).
	pub fn get_friends_interface(&self) -> Option<MutexGuard<FriendsInterface>> {
		self.0.friends_interface.as_ref().map(|mutex| mutex.lock().unwrap())
	}

	/// # Panics
	/// If the interface was not enabled during [`init`](Self::init).
	pub fn friends_interface(&self) -> MutexGuard<FriendsInterface> {
		self.get_friends_interface().expect("FriendsInterface must be enabled")
	}

	/// Returns `None` if the interface was not enabled during [`init`](Self::init).
	pub fn get_user_stats_interface(&self) -> Option<MutexGuard<UserStatsInterface>> {
		self.0.user_stats_interface.as_ref().map(|mutex| mutex.lock().unwrap())
	}

	/// # Panics
	/// If the interface was not enabled during [`init`](Self::init).
	pub fn user_stats_interface(&self) -> MutexGuard<UserStatsInterface> {
		self.get_user_stats_interface().expect("UserStatsInterface must be enabled")
	}
}
...
pub struct SteamworksInterfaceConfig {
	...
	/// Set to `true` if integration with Steam friends is needed.
	///
	...
	/// Set to `true` if access to users' stats is needed.
	///
