...
fn main() {
	...
	let mut redis_name: Option<&str> = None;
	...
	for file_name in &copied_file_names {
		...
		if extension == std::env::consts::DLL_EXTENSION {
			redis_name = Some(stem);

			break;
		}
	}

	let redis_name = redis_name.expect("missing Steam API DLL - this target may not be supported");
	...
	#[cfg(feature = "generate_bindings")]
	{
		...
		let bindgen_dt = bindgen::Builder::default()
			.headers([
				...
				headers_dir.join("isteamtimeline.h").to_string_lossy(),
			])
